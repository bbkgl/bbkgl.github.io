...
此时肯定注意到了 `Z_Construct_UClass_UHH_Statics`，`ClassParams` 正是该结构体中的一个静态成员：

![1613037320097](/cloud_img/1613037320097.png)

也就是说，属性最终会注册到 `Z_Construct_UClass_UHH_Statics::ClassParams` 中，关于  `Z_Construct_UClass_UHH_Statics`，先挖坑，后面接着讲。
...
`UHH::StaticRegisterNativesUHH()` 会传给 `UHH::GetPrivateStaticClass()`，在`GetPrivateStaticClassBody()` 函数中调用。
