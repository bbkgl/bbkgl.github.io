...
**2. 为什么存在`__DefaultConstructor()` 构造函数？**

这是因为通过名字动态生成对象，而构造函数无法使用函数指针指向，因此这里就用一个static函数包装一下，变成一个"平凡"的函数指针，而且所有类的签名一致，就可以在UClass里用一个函数指针里保存起来。
